...
void RigRenderer::init(SDL_GPUDevice *device,
...

    {
        ...
        SDL_GPUShader *vert = am->load_shader(
            "rig_terrain.vert", shader_dir + "/terrain.vert.glsl.spv",
            SDL_GPU_SHADERSTAGE_VERTEX, 1, 0);
        SDL_GPUShader *frag = am->load_shader(
            "rig_terrain.frag", shader_dir + "/terrain.frag.glsl.spv",
            SDL_GPU_SHADERSTAGE_FRAGMENT, 1, 3);

        if (!vert || !frag) {
            SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                         "RigRenderer: Failed to load terrain shaders");
            return;
        }
        ...
        pi.rasterizer_state.depth_bias_constant_factor   = -1.0f;
        pi.rasterizer_state.depth_bias_slope_factor      = -1.0f;

        pipeline = SDL_CreateGPUGraphicsPipeline(device, &pi);
        if (!pipeline) {
            SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                         "RigRenderer: Failed to create pipeline: %s", SDL_GetError());
            return;
        }
        SDL_Log("RigRenderer: Created dedicated pipeline with depth bias");
    }
...
uint32_t RigRenderer::prepare(SDL_GPUCommandBuffer *cmd, flecs::world &ecs) {
    ...
    glm::vec3 ik_goal_color(0.2f, 0.8f, 0.2f);
    glm::vec3 pole_color(0.9f, 0.5f, 0.1f);
    ...
    ecs.each([&](ActorTag, const RigPose &pose_in, const ActorConfig &cfg,
        ...
        {
            glm::vec3 trace_color(0.3f, 0.85f, 0.85f);
