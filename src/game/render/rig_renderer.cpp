...
void RigRenderer::cleanup(SDL_GPUDevice *device) {
    ...
    // Skinned mesh cleanup
    if (skinned_pipeline) { SDL_ReleaseGPUGraphicsPipeline(device, skinned_pipeline); skinned_pipeline = nullptr; }
    if (skinned_vbo)      { SDL_ReleaseGPUBuffer(device, skinned_vbo);               skinned_vbo = nullptr; }
    if (skinned_ibo)      { SDL_ReleaseGPUBuffer(device, skinned_ibo);               skinned_ibo = nullptr; }
    if (joint_ssbo)       { SDL_ReleaseGPUBuffer(device, joint_ssbo);                joint_ssbo = nullptr; }
    if (joint_transfer)   { SDL_ReleaseGPUTransferBuffer(device, joint_transfer);    joint_transfer = nullptr; }
    skinned_initialized = false;
}
...
bool RigRenderer::init_skinned(SDL_GPUDevice *device, SDL_Window *window,
                                SDL_GPUTextureFormat depth_fmt,
                                AssetManager *am, const std::string &glb_path) {
    if (skinned_initialized) return true;

    // 1. Load glTF
    GltfAsset asset = load_gltf(glb_path);
    if (!asset.ok || asset.skins.empty()) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                     "init_skinned: failed to load %s", glb_path.c_str());
        return false;
    }

    // 2. Find first skinned mesh
    const GltfMeshData *skinned_mesh = nullptr;
    for (const auto &m : asset.meshes) {
        if (m.has_skinning()) { skinned_mesh = &m; break; }
    }
    if (!skinned_mesh) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "init_skinned: no skinned mesh found");
        return false;
    }

    // 3. Build skinned mesh data
    SkinnedMeshData mesh_data = build_skinned_mesh(*skinned_mesh, asset.skins[0]);
    if (mesh_data.vertices.empty()) return false;

    // 4. Build skeleton def (with xyzw->wxyz quat conversion)
    skeleton_def = build_skeleton_def(asset.skins[0]);

    // 5. Build joint mapping + bind-pose calibration
    ActorConfig default_cfg{};
    RigTransforms bind_pose_xforms = compute_procedural_bind_pose(default_cfg);

    if (!build_joint_mapping(joint_mapping, asset.skins[0], skeleton_def, bind_pose_xforms)) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "init_skinned: joint mapping failed");
        return false;
    }

    skinned_joint_count = mesh_data.joint_count;
    joint_palette_cache.resize(skinned_joint_count, glm::mat4(1.0f));

    // 6. Upload static VBO + IBO
    {
        uint32_t vbo_size = (uint32_t)(mesh_data.vertices.size() * sizeof(SkinnedVertex));
        SDL_GPUBufferCreateInfo bi = {};
        bi.usage = SDL_GPU_BUFFERUSAGE_VERTEX;
        bi.size  = vbo_size;
        skinned_vbo = SDL_CreateGPUBuffer(device, &bi);

        uint32_t ibo_size = (uint32_t)(mesh_data.indices.size() * sizeof(uint32_t));
        bi.usage = SDL_GPU_BUFFERUSAGE_INDEX;
        bi.size  = ibo_size;
        skinned_ibo = SDL_CreateGPUBuffer(device, &bi);
        skinned_index_count = (uint32_t)mesh_data.indices.size();

        // Upload via transfer buffer
        SDL_GPUTransferBufferCreateInfo tbi = {};
        tbi.usage = SDL_GPU_TRANSFERBUFFERUSAGE_UPLOAD;
        tbi.size  = std::max(vbo_size, ibo_size);
        SDL_GPUTransferBuffer *upload = SDL_CreateGPUTransferBuffer(device, &tbi);

        // VBO upload
        void *ptr = SDL_MapGPUTransferBuffer(device, upload, false);
        std::memcpy(ptr, mesh_data.vertices.data(), vbo_size);
        SDL_UnmapGPUTransferBuffer(device, upload);
        SDL_GPUCommandBuffer *cmd = SDL_AcquireGPUCommandBuffer(device);
        SDL_GPUCopyPass *cp = SDL_BeginGPUCopyPass(cmd);
        SDL_GPUTransferBufferLocation src = { upload, 0 };
        SDL_GPUBufferRegion dst = { skinned_vbo, 0, vbo_size };
        SDL_UploadToGPUBuffer(cp, &src, &dst, false);
        SDL_EndGPUCopyPass(cp);
        ...
        // IBO upload
        ptr = SDL_MapGPUTransferBuffer(device, upload, false);
        std::memcpy(ptr, mesh_data.indices.data(), ibo_size);
        SDL_UnmapGPUTransferBuffer(device, upload);
        cmd = SDL_AcquireGPUCommandBuffer(device);
        cp = SDL_BeginGPUCopyPass(cmd);
        src = { upload, 0 };
        dst = { skinned_ibo, 0, ibo_size };
        SDL_UploadToGPUBuffer(cp, &src, &dst, false);
        SDL_EndGPUCopyPass(cp);
        SDL_SubmitGPUCommandBuffer(cmd);
        SDL_WaitForGPUIdle(device);
    ...
    // 7. Create joint SSBO + transfer buffer
    {
        uint32_t palette_bytes = skinned_joint_count * (uint32_t)sizeof(glm::mat4);
        SDL_GPUBufferCreateInfo bi = {};
        bi.usage = SDL_GPU_BUFFERUSAGE_GRAPHICS_STORAGE_READ;
        bi.size  = palette_bytes;
        joint_ssbo = SDL_CreateGPUBuffer(device, &bi);

        SDL_GPUTransferBufferCreateInfo tbi = {};
        tbi.usage = SDL_GPU_TRANSFERBUFFERUSAGE_UPLOAD;
        tbi.size  = palette_bytes;
        joint_transfer = SDL_CreateGPUTransferBuffer(device, &tbi);
    }

    // 8. Create pipeline
    if (!init_skinned_pipeline(device, window, depth_fmt, am)) return false;

    skinned_initialized = true;
    SDL_Log("RigRenderer: skinned mesh ready (%u verts, %u indices, %u joints, %d mapped)",
            (uint32_t)mesh_data.vertices.size(), skinned_index_count,
            skinned_joint_count, joint_mapping.mapped_count);
    return true;
}

bool RigRenderer::init_skinned_pipeline(SDL_GPUDevice *device, SDL_Window *window,
    ...
    // Vertex: 1 UBO (SceneUniforms), 1 SSBO (JointBuffer)
    SDL_GPUShader *vert = am->load_shader(
        "pbr_skinned.vert", shader_dir + "/pbr_skinned.vert.glsl.spv",
        SDL_GPU_SHADERSTAGE_VERTEX, 1, 1);
    // Fragment: 1 UBO (SceneUniforms), 3 SSBOs (lights)
    SDL_GPUShader *frag = am->load_shader(
        "pbr_static.frag", shader_dir + "/pbr_static.frag.glsl.spv",
        SDL_GPU_SHADERSTAGE_FRAGMENT, 1, 3);

    if (!vert || !frag) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "init_skinned_pipeline: shader load failed");
        return false;
    }

    // SkinnedVertex: 72 bytes, 8 vertex attributes
    ...
    attrs[0] = { 0, 0, SDL_GPU_VERTEXELEMENTFORMAT_FLOAT3,
                 (Uint32)offsetof(SkinnedVertex, pos_x) };
    attrs[1] = { 1, 0, SDL_GPU_VERTEXELEMENTFORMAT_FLOAT3,
                 (Uint32)offsetof(SkinnedVertex, nx) };
    attrs[2] = { 2, 0, SDL_GPU_VERTEXELEMENTFORMAT_FLOAT2,
                 (Uint32)offsetof(SkinnedVertex, u) };
    attrs[3] = { 3, 0, SDL_GPU_VERTEXELEMENTFORMAT_FLOAT3,
                 (Uint32)offsetof(SkinnedVertex, color_r) };
    attrs[4] = { 4, 0, SDL_GPU_VERTEXELEMENTFORMAT_FLOAT,
                 (Uint32)offsetof(SkinnedVertex, roughness) };
    attrs[5] = { 5, 0, SDL_GPU_VERTEXELEMENTFORMAT_FLOAT,
                 (Uint32)offsetof(SkinnedVertex, metallic) };
    attrs[6] = { 6, 0, SDL_GPU_VERTEXELEMENTFORMAT_UBYTE4,
                 (Uint32)offsetof(SkinnedVertex, joint_indices) };
    attrs[7] = { 7, 0, SDL_GPU_VERTEXELEMENTFORMAT_FLOAT4,
                 (Uint32)offsetof(SkinnedVertex, weight_x) };
    ...
    pi.vertex_input_state.num_vertex_attributes      = 8;
    ...
    skinned_pipeline = SDL_CreateGPUGraphicsPipeline(device, &pi);
    if (!skinned_pipeline) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                     "init_skinned_pipeline: pipeline creation failed: %s", SDL_GetError());
        return false;
    }

    SDL_Log("RigRenderer: skinned pipeline created");
    return true;
}

bool RigRenderer::prepare_skinned(SDL_GPUCommandBuffer *cmd, flecs::world &ecs) {
    if (!skinned_initialized) return false;

    bool found = false;
    ecs.each([&](ActorTag, const RigPose &pose, const ActorConfig &cfg,
                 const Transform &t, const ProceduralGait &gait, const RigState &anim,
                 const RigTransforms &xforms) {
        if (found) return;  // single actor
        found = true;

        // 1. Compute hip counter-animation
        ...
        // 2. Build modified xforms with hip bob + iso foreshortening
        RigTransforms modified_xforms = xforms;

        // Apply hip bob to mapped bone positions
        glm::vec3 hip_bob_offset(0.0f, 0.0f, hip_anim.hip_bob_y);
        for (int ji = 0; ji < (int)Joint::POLE_KNEE_L; ++ji) {
            // Translate bone positions by hip bob (vertical offset)
            modified_xforms.bones[ji][3] += glm::vec4(hip_bob_offset, 0.0f);
        }

        // Apply isometric foreshortening to bone Z positions
        float foot_z = t.z;
        float scale = AnimationConfig::ISO_CHAR_HEIGHT_SCALE;
        for (int ji = 0; ji < (int)Joint::POLE_KNEE_L; ++ji) {
            float orig_z = modified_xforms.bones[ji][3].z;
            modified_xforms.bones[ji][3].z = foot_z + (orig_z - foot_z) * scale;
            // Scale Z component of rotation columns for foreshortening
            modified_xforms.bones[ji][0].z *= scale;
            modified_xforms.bones[ji][1].z *= scale;
            modified_xforms.bones[ji][2].z *= scale;
        }

        // 3. Compute skinning palette
        compute_skinning_palette(modified_xforms, joint_mapping, skeleton_def, joint_palette_cache);

        // 4. Upload palette to SSBO
        uint32_t palette_bytes = skinned_joint_count * (uint32_t)sizeof(glm::mat4);
        void *ptr = SDL_MapGPUTransferBuffer(gpu_device, joint_transfer, true);
        if (!ptr) { found = false; return; }
        std::memcpy(ptr, joint_palette_cache.data(), palette_bytes);
        SDL_UnmapGPUTransferBuffer(gpu_device, joint_transfer);

        SDL_GPUCopyPass *cp = SDL_BeginGPUCopyPass(cmd);
        SDL_GPUTransferBufferLocation src = { joint_transfer, 0 };
        SDL_GPUBufferRegion dst = { joint_ssbo, 0, palette_bytes };
        SDL_UploadToGPUBuffer(cp, &src, &dst, false);
        SDL_EndGPUCopyPass(cp);
    });
...
void RigRenderer::draw_skinned(SDL_GPURenderPass *pass, SDL_GPUCommandBuffer *cmd,
                                const SceneUniforms &uniforms,
                                SDL_GPUBuffer *point_light_ssbo,
                                SDL_GPUBuffer *light_grid_ssbo,
                                SDL_GPUBuffer *global_index_ssbo) {
    if (!skinned_initialized || skinned_index_count == 0) return;

    SDL_BindGPUGraphicsPipeline(pass, skinned_pipeline);

    // Push SceneUniforms to both stages
    SDL_PushGPUVertexUniformData(cmd, 0, &uniforms, sizeof(SceneUniforms));
    SDL_PushGPUFragmentUniformData(cmd, 0, &uniforms, sizeof(SceneUniforms));

    // Vertex storage: JointBuffer (set=0, binding=0)
    SDL_GPUBuffer *vert_storage[1] = { joint_ssbo };
    SDL_BindGPUVertexStorageBuffers(pass, 0, vert_storage, 1);

    // Fragment storage: clustered lighting (set=2, bindings 0-2)
    SDL_GPUBuffer *frag_storage[3] = {
        point_light_ssbo  ? point_light_ssbo  : dummy_ssbo_,
        light_grid_ssbo   ? light_grid_ssbo   : dummy_ssbo_,
        global_index_ssbo ? global_index_ssbo  : dummy_ssbo_,
    };
    SDL_BindGPUFragmentStorageBuffers(pass, 0, frag_storage, 3);

    // Bind VBO + IBO
    SDL_GPUBufferBinding vbind = { skinned_vbo, 0 };
    SDL_BindGPUVertexBuffers(pass, 0, &vbind, 1);
    SDL_GPUBufferBinding ibind = { skinned_ibo, 0 };
    SDL_BindGPUIndexBuffer(pass, &ibind, SDL_GPU_INDEXELEMENTSIZE_32BIT);

    SDL_DrawGPUIndexedPrimitives(pass, skinned_index_count, 1, 0, 0, 0);
}
