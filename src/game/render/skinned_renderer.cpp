...
bool SkinnedRenderer::build_pipeline(SDL_Window *window) {
  ...
  SDL_GPUTextureFormat sc_fmt =
      SDL_GetGPUSwapchainTextureFormat(device_, window);

  SDL_GPUShader *vert = assets_->load_shader(
      "skinned_char.vert", shader_dir + "/skinned_character.vert.glsl.spv",
  ...
  if (!vert || !frag) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                 "SkinnedRenderer: failed to load shaders");
    return false;
  }
  ...
  attrs[0] = {0, 0, SDL_GPU_VERTEXELEMENTFORMAT_FLOAT3,
              (Uint32)offsetof(SkinnedVertex, position)};
  attrs[1] = {1, 0, SDL_GPU_VERTEXELEMENTFORMAT_FLOAT3,
              (Uint32)offsetof(SkinnedVertex, normal)};
  attrs[2] = {2, 0, SDL_GPU_VERTEXELEMENTFORMAT_FLOAT2,
              (Uint32)offsetof(SkinnedVertex, texcoord)};
  attrs[3] = {3, 0, SDL_GPU_VERTEXELEMENTFORMAT_FLOAT4,
              (Uint32)offsetof(SkinnedVertex, tangent)};
  attrs[4] = {4, 0, SDL_GPU_VERTEXELEMENTFORMAT_UBYTE4,
              (Uint32)offsetof(SkinnedVertex, joints)};
  attrs[5] = {5, 0, SDL_GPU_VERTEXELEMENTFORMAT_FLOAT4,
              (Uint32)offsetof(SkinnedVertex, weights)};
  ...
  if (!pipeline_) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                 "SkinnedRenderer: pipeline creation failed: %s",
                 SDL_GetError());
    return false;
  }

  assets_->register_pipeline("skinned_char", "skinned_char.vert",
                             "skinned_char.frag");
  SDL_Log("SkinnedRenderer: pipeline created (stride=%zu)",
          sizeof(SkinnedVertex));
  return true;
}

void SkinnedRenderer::init(SDL_GPUDevice *device, SDL_Window *window,
                           AssetManager *assets,
  ...
  if (!bone_ssbo_) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                 "SkinnedRenderer: failed to create bone SSBO");
    return;
  }
  ...
  if (!bone_transfer_) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                 "SkinnedRenderer: failed to create bone transfer buf");
...
void SkinnedRenderer::cleanup() {
  ...
  if (vbo_) {
    SDL_ReleaseGPUBuffer(device_, vbo_);
    vbo_ = nullptr;
  }
  if (ibo_) {
    SDL_ReleaseGPUBuffer(device_, ibo_);
    ibo_ = nullptr;
  }
  if (bone_ssbo_) {
    SDL_ReleaseGPUBuffer(device_, bone_ssbo_);
    bone_ssbo_ = nullptr;
  }
  if (bone_transfer_) {
    SDL_ReleaseGPUTransferBuffer(device_, bone_transfer_);
    bone_transfer_ = nullptr;
  }
  initialized_ = false;
  char_loaded_ = false;
}
...
void SkinnedRenderer::load_character(const std::string &path) {
  ...
  if (!asset.ok) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                 "SkinnedRenderer: failed to load '%s': %s", path.c_str(),
                 asset.error.c_str());
    return;
  }
  ...
  for (auto &mesh : asset.meshes) {
    ...
    all_verts.insert(all_verts.end(), mesh.vertices.begin(),
                     mesh.vertices.end());
  ...
  if (!vbo_ || !ibo_) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                 "SkinnedRenderer: VBO/IBO upload failed");
    return;
  }
  ...
  SDL_Log(
      "SkinnedRenderer: BoneMap — hips=%d spine=%d chest=%d neck=%d head=%d",
      bone_map_.hips, bone_map_.spine, bone_map_.chest, bone_map_.neck,
      bone_map_.head);
...
void SkinnedRenderer::load_animation(const std::string &name,
                                     const std::string &path) {
  GltfSkinnedAsset asset = load_gltf_skinned(path);
  if (!asset.ok) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                 "SkinnedRenderer: failed to load anim '%s': %s", path.c_str(),
                 asset.error.c_str());
    return;
  }
...
void SkinnedRenderer::select_clip(const std::string &name,
...
void SkinnedRenderer::prepare(SDL_GPUCommandBuffer *cmd) {
  if (!initialized_ || !char_loaded_ || !bone_transfer_ || !bone_ssbo_)
    return;
...
void SkinnedRenderer::draw(SDL_GPURenderPass *pass, SDL_GPUCommandBuffer *cmd,
  ...
  if (!initialized_ || !char_loaded_ || !vbo_ || !ibo_)
    return;
  if (!pipeline_) {
    SDL_Log("SkinnedRenderer: pipeline null, skipping draw");
    return;
  }
  ...
  SDL_GPUTextureSamplerBinding tsb[2] = {{light_tex, light_smp},
                                         {fluence_tex, fluence_smp}};
